...
public class Thieving extends ItemIdentifiers {
	...
	public static final class StallThieving {
		...
		public static boolean init(Player player, GameObject object) {
			...
			if (CombatFactory.inCombat(player)) {
				player.getPacketSender().sendMessage("You need to be out of combat!");
				return true;
			}
			if (stall.isPresent()) {
				...
				if (player.getSkillManager().getCurrentLevel(Skill.THIEVING) >= stall.get().getReqLevel()) {
					...
					//	boolean isSuccess = false;
						...
						// Perform animation..
						player.performAnimation(THIEVING_ANIMATION);
						int lvl = player.getSkillManager().getMaxLevel(Skill.THIEVING);
						int stalllvl = stall.get().getReqLevel();
						...
						// Respawn stall..
						if (giveItem(player, stall,
								lvl < stalllvl + 5 ? UNCOMMON.success() :
										lvl < stalllvl + 15 ? SOMETIMES.success() :
												lvl < stalllvl + 25 ? COMMON.success() :
														VERY_COMMON.success())){
					...
					player.getPacketSender().sendMessage("You need a Thieving level of at least "
							+ stall.get().getReqLevel() + " to do this.");
				}
				return true;
			}
			return false;
		...
		private static boolean giveItem(Player player, Optional<Stall> stall, boolean success){
			if(success) {
				Item item = stall.get().getRewards()[Misc.getRandom(stall.get().getRewards().length - 1)];
				player.getInventory().add(item.getId(), item.getAmount() > 1 ? Misc.getRandom(item.getAmount()) : 1);
				return true;
			} else {
		...
		public enum Stall {
			...
			CRAFTING_STALL(new StallDefinition[]{new StallDefinition(4874, Optional.empty()),
					new StallDefinition(6166, Optional.empty())},
					5, 16, 12, new Item(CHISEL), new Item(RING_MOULD), new Item(NECKLACE_MOULD)),
			MONKEY_STALL(new StallDefinition[]{new StallDefinition(4875, Optional.of(620))}, 5, 16, 12,
					new Item(BANANA)),
			MONKEY_GENERAL_STALL(
					new StallDefinition[]{new StallDefinition(4876, Optional.of(620))}, 5,
					16, 12, new Item(POT), new Item(TINDERBOX), new Item(HAMMER)),
			...
			WINE_STALL(new StallDefinition[]{new StallDefinition(14011,
					Optional.of(634))
			},
					22, 27, 27,
					new Item(JUG_OF_WATER),
					new Item(JUG_OF_WINE),
					new Item(GRAPES),
					new Item(EMPTY_JUG),
					new Item(BOTTLE_OF_WINE)),
			VEG_STALL(new StallDefinition[]{new StallDefinition(
					4706, Optional.of(634)),
			},
					27, 10, 30,
			...
			FUR_STALL(new StallDefinition[]{
					new StallDefinition(11732, Optional.of(634)),
			...
			FISH_STALL(new StallDefinition[]{
					...
					new StallDefinition(4705, Optional.of(4276))
			},
					42, 42, 17,
					new Item(RAW_SALMON),
					new Item(RAW_TUNA)),
			CROSSBOW_STALL(new StallDefinition[]{
					new StallDefinition(17031, Optional.of(6984))
			},
					49, 52, 15,
					new Item(BRONZE_BOLTS, 6),
					new Item(BRONZE_LIMBS),
					new Item(WOODEN_STOCK)),
			SILVER_STALL(new StallDefinition[]{
					new StallDefinition(11734, Optional.of(634)),
					new StallDefinition(6164, Optional.of(6984)),
			},
					50, 54, 50,
					new Item(SILVER_ORE)),
			SPICE_STALL(new StallDefinition[]{
					new StallDefinition(11733, Optional.of(634)),
					new StallDefinition(6572, Optional.of(6573)),
					new StallDefinition(20348, Optional.of(20349)),
			},
					65, 81, 133,
					new Item(SPICE)),
			MAGIC_STALL(new StallDefinition[]{
					new StallDefinition(4877, Optional.of(620)),
			},
					65, 100, 133,
