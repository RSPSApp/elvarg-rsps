...
/**
 * @author Ynneh | 07/02/2023 - 22:51
 * <https://github.com/drhenny>
 */
public class Chain<T> {

    private static final Logger logger = Logger.getLogger(String.valueOf(Chain.class));
     ...
     * @return
     */
    public Chain<T> waitForTile(Location tile, Runnable work) {
        if (this.work != null) {
        ...
        executeCondition = () -> ((Entity)owner).getLocation().getX() == tile.getX() && ((Entity)owner).getLocation().getY() == tile.getY();
        cycleDelay = 1;
        this.work = work;
        repeats = true;
        startChainExecution();
    ...
    private void startChainExecution() {
        if (cycleDelay == 0) {
            ...
            task = new Task(cycleDelay, name != null ? name : "", false) {
                @Override
            ...
            String s = Arrays.toString(fromLocation.stream().map(s1 -> s1.toString())
                    .map(s2 -> {
